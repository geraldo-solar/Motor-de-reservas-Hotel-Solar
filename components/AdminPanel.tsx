...
const GeneralMapCalendar: React.FC<{
  ...
  // Filter out past dates only (admin can edit future dates)
  ...
  const daysArray = allDays.filter(day => {
    ...
    return dateObj >= today; // Permitir edição de todas as datas futuras
  });
...
const AdminPanel: React.FC<AdminPanelProps> = ({ 
  ...
  const navItemClass = (tab: AdminTab) => 
    `flex items-center gap-2 px-3 py-2 rounded-lg transition-all text-sm ${activeTab === tab 
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#F9F8F6]">
      ...
      <header className="bg-[#0F2820] text-[#E5D3B3] shadow-lg sticky top-0 z-50">
        <div className="flex items-center justify-between px-4 py-3">
          {/* Logo/Title */}
          <div className="flex items-center gap-3">
            <Lock className="text-[#D4AF37]" size={24} />
            <div>
              <h1 className="font-serif font-bold tracking-widest text-xl">SOLAR ADMIN</h1>
          ...
          <nav className="hidden lg:flex items-center gap-2">
            <a 
                ...
                className="flex items-center gap-2 px-3 py-2 rounded-lg transition-all text-[#E5D3B3] hover:bg-[#2F3A2F] border border-[#4A5D43] text-sm"
            >
                <ArrowLeft size={16} /> Site
            </a>
            <button onClick={() => setActiveTab('map')} className={navItemClass('map')}>
              <Grid size={16} /> Mapa
            </button>
            <button onClick={() => setActiveTab('reservations')} className={navItemClass('reservations')}>
              <BookCheck size={16} /> Reservas
            </button>
            <button onClick={() => setActiveTab('rooms')} className={navItemClass('rooms')}>
              <BedDouble size={16} /> Acomodações
            </button>
            <button onClick={() => setActiveTab('packages')} className={navItemClass('packages')}>
              <Package size={16} /> Pacotes
            </button>
            <button onClick={() => setActiveTab('extras')} className={navItemClass('extras')}>
              <ShoppingBag size={16} /> Extras
            </button>
            <button onClick={() => setActiveTab('discounts')} className={navItemClass('discounts')}>
              <Tag size={16} /> Cupons
            </button>
          ...
          <div className="flex items-center gap-2">
            ...
            <button 
              onClick={onLogout}
              className="hidden md:flex items-center gap-2 bg-red-900/30 text-red-400 px-3 py-2 rounded hover:bg-red-900/50 transition text-sm"
            >
              <LogOut size={16} /> Sair
            </button>
            
            {/* Mobile Menu Toggle */}
            <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="lg:hidden p-2 bg-[#2F3A2F] rounded text-[#D4AF37]">
        ...
        {isMobileMenuOpen && (
          <div className="lg:hidden bg-[#0A1C16] border-t border-[#4A5D43] p-4 space-y-2">
            <a 
                href="https://www.hotelsolar.tur.br/"
                className="flex items-center gap-2 px-4 py-3 rounded-lg transition-all text-[#E5D3B3] hover:bg-[#2F3A2F] border border-[#4A5D43] w-full"
            >
                <ArrowLeft size={18} /> Voltar para site
            </a>
            <button onClick={() => handleMobileNav('map')} className={`${navItemClass('map')} w-full`}>
              <Grid size={18} /> Mapa Geral
            </button>
            <button onClick={() => handleMobileNav('reservations')} className={`${navItemClass('reservations')} w-full`}>
              <BookCheck size={18} /> Reservas
            </button>
            <button onClick={() => handleMobileNav('rooms')} className={`${navItemClass('rooms')} w-full`}>
              <BedDouble size={18} /> Acomodações
            </button>
            <button onClick={() => handleMobileNav('packages')} className={`${navItemClass('packages')} w-full`}>
              <Package size={18} /> Pacotes
            </button>
            <button onClick={() => handleMobileNav('extras')} className={`${navItemClass('extras')} w-full`}>
              <ShoppingBag size={18} /> Produtos Extras
            </button>
            <button onClick={() => handleMobileNav('discounts')} className={`${navItemClass('discounts')} w-full`}>
              <Tag size={18} /> Cupons
            </button>
            <button onClick={() => handleMobileNav('ai')} className={`${navItemClass('ai')} w-full`}>
              <BrainCircuit size={18} /> Assistente IA
            </button>
            <button onClick={() => handleMobileNav('config')} className={`${navItemClass('config')} w-full`}>
              <Settings size={18} /> Configurações
            </button>
            <button 
              onClick={onLogout}
              className="w-full flex items-center justify-center gap-2 bg-red-900/30 text-red-400 px-4 py-3 rounded hover:bg-red-900/50 transition"
            >
              <LogOut size={18} /> Sair
            </button>
          </div>
